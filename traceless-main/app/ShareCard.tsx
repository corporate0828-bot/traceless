...
const ShareCard = () => {
  ...
  const socket = userDetails.socket;
  ...
  return (
    <>
      <Card className="w-full sm:max-w-[480px] max-w-[95%] z-10 border rounded-xl shadow-lg mx-auto">
        <CardHeader className="pb-3">
          <CardTitle className="text-2xl">TraceLess</CardTitle>
          <CardDescription className="text-sm">
            Privacy-first peer transfer for shared and public systems.
          </CardDescription>
        </CardHeader>
        <CardContent className="mt-2 space-y-5">
          <form className="space-y-5">
            <div className="flex flex-col gap-y-2">
              <Label htmlFor="name" className="text-base font-semibold">Your TraceLess token</Label>
              <div className="flex flex-col sm:flex-row gap-2 items-stretch sm:items-center">
                <div className="flex border rounded-lg px-3 py-2.5 flex-1 bg-muted font-mono text-xs break-all items-center">
              ...
              <div className="mt-4 flex flex-col gap-y-2">
                <Label className="text-base font-semibold">Scan QR to connect</Label>
                <div className="flex items-center justify-center rounded-lg border border-border bg-muted p-4">
                  {userId ? (
                    <QRCodeSVG
                      value={`${process.env.NEXT_PUBLIC_APP_URL?.replace(/\/$/, "") || (typeof window !== "undefined" ? window.location.origin : "http://localhost:3000")}/share?code=${userId}`}
                      size={160}
            ...
            <div className="flex flex-col gap-y-2">
                <Label htmlFor="name" className="text-base font-semibold">Peer token</Label>
                <div className="flex flex-col sm:flex-row gap-2 items-stretch sm:items-center">
                  <Input
                    id="name"
                    placeholder="Enter a peer token"
                  ...
                  <Button
                    type="button"
                    variant="outline"
                    className="px-4 py-2.5 sm:py-2.5 rounded-lg w-full sm:w-[180px] font-medium"
              ...
              <div className="flex flex-col gap-y-2">
                <Label htmlFor="name" className="text-base font-semibold">Connection status</Label>
                <div className="flex flex-col sm:flex-row gap-2 items-stretch sm:items-center">
                  <div className="border rounded-lg px-3 py-2.5 text-sm flex-1 flex items-center ease-in-out duration-500 transition-all select-none bg-muted">
                    {currentConnection
                  ...
                  <>
                    {terminateCall ? (
                      <Button
                        variant="destructive"
                        type="button"
                        className="px-4 py-2.5 rounded-lg w-full sm:w-auto font-medium"
                        onClick={() => {
                          peerRef.current.destroy();
                        }}
                      >
              ...
              {/* file upload */}
              <div className="flex flex-col border rounded-lg px-3 py-3 text-sm w-full ease-in-out duration-500 transition-all gap-y-3 bg-background">
                <div>
